...
final class KeyedSemaphore
{
    ...
    /**
     * Check if the semaphore has reached it's limit for the given key.
     *
     * If this method returns `true`, it means future calls to `waitFor` with the given key will wait before executing operation.
     * If this method return `false`, it means future calls to `waitFor` with the given key will be executed immediately.
     *
     * @param Tk $key
     */
    public function hasReachedLimit(string|int $key): bool
    {
        return ($this->ingoing[$key] ?? 0) === $this->concurrencyLimit;
    }
}

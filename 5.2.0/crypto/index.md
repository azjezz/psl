...
$ciphertext = Symmetric\seal('secret payload', $key, additional_data: 'user-id:42');

// Decryption succeeds only if the same AAD is provided
$plaintext = Symmetric\open($ciphertext, $key, additional_data: 'user-id:42');
IO\write_line('Decrypted: %s', $plaintext);

// Wrong or missing AAD causes decryption to fail
try {
    Symmetric\open($ciphertext, $key, additional_data: 'user-id:99');
...
See [src/Psl/Crypto/](https://github.com/php-standard-library/php-standard-library/tree/5.2.0/src/Psl/Crypto/) for the full API.

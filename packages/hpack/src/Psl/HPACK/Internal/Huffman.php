...
final class Huffman
{
    ...
    /**
     * Bit lengths per symbol, cached from CODE_TABLE.
     *
     * @var null|array<int<0, 255>, int>
     */
    private static null|array $bitLengths = null;

    /**
     * @return array<int<0, 255>, int>
     */
    private static function getBitLengths(): array
    {
        if (self::$bitLengths === null) {
            self::$bitLengths = [];
            for ($i = 0; $i < 256; $i++) {
                self::$bitLengths[$i] = self::CODE_TABLE[$i][1];
            }
        }

        return self::$bitLengths;
    }
    ...
    {
        $bitLengths = self::getBitLengths();
        $bits = 0;
        for ($i = 0; $i < $length; $i++) {
            $bits += $bitLengths[ord($data[$i])];
        }

...
/**
 * Lazy decompressing read decorator.
 *
...
final class DecompressingReadHandle implements IO\ReadHandleInterface
{
     ...
     *
     * @throws Exception\RuntimeException If a decompression fails.
    ...
    /**
     * Whether all data from the inner handle has been read and decompressed.
     *
     * Returns true only after the inner handle reaches EOF and the decompressor
    ...
    /**
     * Process a chunk from the inner handle through the decompressor.
     *
     * An empty chunk signals EOF on the inner handle, triggering finalization
     * of the decompression stream.
     *
    ...
    private function processChunk(string $chunk): void
    {
        ...
        $this->buffer .= $this->decompressor->push($chunk);
    }

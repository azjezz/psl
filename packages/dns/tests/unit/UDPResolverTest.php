...
final class UDPResolverTest extends TestCase
{
    ...
    public function testTruncatedResponseThrowsProtocolException(): void
    {
        ...
        \Psl\Async\run(static function () use ($socket): void {
            [$data, $peer] = $socket->receiveFrom(512);
            $id = new \Psl\Binary\Reader($data)->u16();
        ...
        try {
            $resolver->query('example.com', RecordType::A);
            static::fail('Expected ProtocolException for truncated response');
        } catch (ProtocolException $e) {
            static::assertStringContainsString('truncat', Str\Byte\lowercase($e->getMessage()));
        } finally {
            $socket->close();
        }
    }
